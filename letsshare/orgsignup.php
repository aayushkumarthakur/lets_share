...
function emailpass(){
    if(isset($GLOBALS['email']) && isset($GLOBALS['password'])){
        ...
        echo "<br> Password and email are mandatory";
    }
}
function verify(){
    if(!empty(trim($GLOBALS['email'])) && !empty(trim($GLOBALS['password']) && !empty(trim($GLOBALS['head'])) && !empty(trim($GLOBALS['contact'])) && !empty(trim($GLOBALS['cpassword'])))){
         
    } else {
        echo "<br>Make sure all the fields are not empty";
    }
}
function contact(){
...
if(isset($_POST['signup'])){
       if(isset($email) && isset($password)){
           if($password==$cpassword){
            if(!empty(trim($email)) && !empty(trim($password)) && !empty(trim($head)) && !empty(trim($contact)) && !empty(trim($cpassword))){
                if(is_numeric($contact)){
                if($con){
                    $sql = "SELECT * FROM organisation WHERE email = '$email'";
                    $query = mysqli_query($con, $sql);
                     $sqlc = "SELECT * FROM registertable WHERE email = '$email'";
...
<head>
    ...
    <body><br><br>
        <div class=header>Create an organisation profile</div>
        <center>(Please Provide Genuine Information)</center>
        <br><br>
        <center>
    <div class="parent-box">
        ...
        <input name="head" type="text" required placeholder="Name" value = "<?php echo @$_POST['head']; ?>"><br><br>
